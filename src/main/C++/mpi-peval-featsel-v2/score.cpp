...
int load_model(const char* model_path){
	...
	//load P1
	sprintf(model_file_name, "%sP1", model_path);
	ifstream p1model(model_file_name);
	if(!p1model.good()){
		cerr << "error model file " << model_file_name << endl;
		exit(1);
	}
	while (getline(p1model, line)){
    	fea_weight = atof(line.c_str());
    	P1.push_back(fea_weight);
	}
	p1model.close();
	
	//load P2
	sprintf(model_file_name, "%sP2", model_path);
	ifstream p2model(model_file_name);
	if(!p2model.good()){
		cerr << "error model file " << model_file_name << endl;
		exit(1);
	}
	while (getline(p2model, line)){
    	fea_weight = atof(line.c_str());
    	P2.push_back(fea_weight);
	}
	p2model.close();
